...
package external.javabayes.InferenceGraphs;
...
import external.javabayes.BayesianInferences.Explanation;
import external.javabayes.BayesianNetworks.BayesNet;
import external.javabayes.BayesianNetworks.DiscreteVariable;
...
import external.javabayes.InterchangeFormat.IFException;
import external.javabayes.QuasiBayesianInferences.QBExpectation;
import external.javabayes.QuasiBayesianInferences.QBInference;
...
public class InferenceGraph {
 ...
 /**
  ...
  /* 
   * Get the node corresponding to a given variable name. 
   */
  public InferenceGraphNode get_node(String name) {
	InferenceGraphNode retVal = null;
	InferenceGraphNode node;
    for (Enumeration e = nodes.elements(); e.hasMoreElements(); ) {
        node = (InferenceGraphNode)e.nextElement();
        if (node.pv.get_name().equals(name) == true)
        	retVal = node;
    }
    return retVal;
  }
